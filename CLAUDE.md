...
# Run daemon manually
uv run cogent-core                        # foreground; Ctrl+C to stop
COGENT_PORT=8000 uv run cogent-core        # override port
...
This is a **dual-process** local AI agent system. `cogent-core` is a persistent daemon; `cogent` and `cogent-tui` are clients that connect to it over a Unix domain socket.

```
cogent-core (daemon)
  └─ listens on 127.0.0.1:7437 (TCP)
       ↑ JSON-RPC 2.0 NDJSON
cogent (CLI)   cogent-tui (TUI, S2+)
```

**`cogent-tui` is the primary frontend.** All user-facing work on task management, observability, and interaction should be designed for and validated in the TUI first. The `cogent` CLI exists only for quick scripted testing and debugging — it is not a product surface. When implementing features that touch the user interface, invest in the TUI layout, event rendering, and keyboard interactions. Do not shortcut TUI work by pointing to the CLI as an alternative.

### Protocol layer (`src/cogent/core/bus/`)
...
### Transport layer (`src/cogent/core/transport/`)
...
Relevant env vars: `COGENT_CONFIG`, `COGENT_HOST`, `COGENT_PORT`, `COGENT_LOG_LEVEL`, `COGENT_LOG_FILE`, `COGENT_LOG_FORMAT`.

### Daemon entry (`src/cogent/core/app.py`)
...
Integration tests in `tests/conftest.py` spawn a real daemon subprocess using a random free port (via `free_port` fixture). The fixture finds a free port, releases it, passes it to the daemon via `COGENT_PORT`, then polls `asyncio.open_connection` until the daemon is ready.
