...
_PID_FILE = Path.home() / ".cogent" / "cogent-core.pid"


# 尝试连接 daemon，成功则正常返回，失败则抛出 ConnectionRefusedError/OSError
async def _ping_check(config: CogentConfig) -> None:
...
def cmd_core_status(config: CogentConfig) -> None:
    try:
...
def cmd_core_start(config: CogentConfig) -> None:
    try:
        asyncio.run(_ping_check(config))
...
def cmd_core_stop(config: CogentConfig) -> None:
    pid = _running_pid()
    if pid is None:
        print("not running")
        return
