...
class CogentTuiApp(App[None]):
    """Cogent TUI：终端滚屏风格，实时展示 agent 执行过程。"""
    ...
    def compose(self) -> ComposeResult:
        yield Label("[bold]Cogent[/bold]  [dim]connecting...[/dim]", id="header")
    ...
    def _update_header(self, state: str) -> None:
        ...
        header.update(
            f"[bold]Cogent[/bold]  [dim]{self._host}:{self._port}[/dim]"
